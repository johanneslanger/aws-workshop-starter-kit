...
A **team** builds something from scratch, with an AI coding agent as an extra pair of hands. This bundle gives you a group workflow, align → slice → divide → build, plus the conventions and live AWS docs the agent needs.

Works with **any coding agent** that supports skills (Claude Code, or bring your own), in **any AWS account**, a workshop sandbox or your own. Customers can plug in the agent and account they already use.

## Setup

1. **Have a coding agent running.** Bring your own, or, if you're in an AWS sandbox with none installed, set up Claude Code first: see [`../../install/`](../../install/). In a sandbox, start it with permission prompts off for speed (Claude Code: `claude --dangerously-skip-permissions`, see the note below).

2. **Let the agent install the kit, no copy-pasting.** Paste this to your agent:

   > Clone https://github.com/johanneslanger/aws-workshop-starter-kit. From its `bundles/building/` folder, install the four skills (`grill-me`, `to-prd`, `to-issues`, `handoff`) into your skills directory, and copy `AGENTS.md`, `CLAUDE.md`, and `.mcp.json` into my current project. Just do it, don't ask for permission.

You're ready to build.

> **If you're using Claude Code with `--dangerously-skip-permissions`:** it lets the agent run commands and edit files without stopping to ask, exactly what you want for hackathon flow. Use it **only in a throwaway workshop environment** (a Workshop Studio instance or a container that gets torn down). The flag governs the *machine the agent runs on*, not just your AWS account, so **don't use it on your own laptop**, run plain `claude` there, or use [auto mode](https://docs.claude.com/en/docs/claude-code/permission-modes) for fewer prompts with a safety classifier still in the loop. Claude Code refuses the flag when run as root, so launch as a non-root user.
...
3. **Write the PRD.** Run **`to-prd`**. It turns the grilled agreement into a short `PRD.md`, problem, solution, implementation decisions, user stories. This is your shared source of truth.
...
- **to-prd** — write that agreement up as a short `PRD.md` (problem, solution, implementation decisions, user stories)
